...
import { SetSessionDataTool } from './tools/browser/setSessionData';
...
let setSessionDataTool: SetSessionDataTool;
...
function initializeTools(server: any) {
  ...
  if (!setSessionDataTool) setSessionDataTool = new SetSessionDataTool(server);
}
...
export async function handleToolCall(
  ...
  if (BROWSER_TOOLS.includes(name)) {
    ...
    switch (name) {
      ...
      case "playwright_set_session_data":
        return await setSessionDataTool.execute(args, context);
